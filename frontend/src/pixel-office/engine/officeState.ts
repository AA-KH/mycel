import { pickDiversePalette } from '../../utils/paletteUtils';
import {
...
import { getPetCount, getPetName } from '../sprites/petSpriteData';
...
import { CharacterState, Direction, PetState, TILE_SIZE } from '../types';
import { createCharacter, updateCharacter } from './characters';
import { advanceMatrixEffect, startMatrixEffect } from './matrixEffectState';
import { createPet, updatePet } from './petEntity';
import { anchorTile, closestFreeSeat } from './seatPlacement';

/** Wander timer parked on characters the lifecycle script is driving, so the
 *  idle FSM never wanders them away from a queue or a break spot. */
const LIFECYCLE_WANDER_HOLD_SEC = 3600;
...
export class OfficeState {
  ...
  /**
   * Spawn an agent at an explicit tile with NO seat assigned.
   *
   * The lifecycle (`AgentLifecycle`) walks new hires in from the entrance,
   * past HR and the Orchestrator, and only then claims a desk — so the
   * character has to exist before a seat does. Starts inactive and with the
   * wander timer parked so the idle FSM does not drift it off its script.
   */
  spawnAgentAtTile(id: number, col: number, row: number, folderName?: string): void {
    if (this.characters.has(id)) return;
    ...
    ch.isActive = false;
    ch.dir = Direction.UP;
    ch.wanderTimer = LIFECYCLE_WANDER_HOLD_SEC;
    if (folderName) ch.folderName = folderName;
    this.characters.set(id, ch);
  }

  /** Claim a free desk inside the agent's department Area and walk there.
   *  Returns the seat uid, or null when every desk is taken. */
  claimSeatForTeam(id: number, folderName?: string): string | null {
  ...
  /** Park the wander timer so a scripted character stands still between steps. */
  holdStill(id: number): void {
    const ch = this.characters.get(id);
    if (ch) ch.wanderTimer = LIFECYCLE_WANDER_HOLD_SEC;
  }

  /** Face a character in a direction (used when queuing at a desk). */
  faceDirection(id: number, dir: Direction): void {
    const ch = this.characters.get(id);
    if (ch) ch.dir = dir;
  }

  /** True once a character has no path left and is not mid-walk. */
  hasArrived(id: number): boolean {
    const ch = this.characters.get(id);
    if (!ch) return true;
    if (ch.matrixEffect) return false;
    return ch.path.length === 0 && ch.state !== CharacterState.WALK;
  }

  /** True while the spawn/despawn matrix effect is still playing. */
  isMaterializing(id: number): boolean {
    const ch = this.characters.get(id);
    return !!ch && ch.matrixEffect === 'spawn';
  }
